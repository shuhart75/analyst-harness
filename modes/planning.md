...
- Personal coefficients and closed intervals come from `planning/team.md`.
...
- Use `planning/team.md` as the team roster. Default lanes are `A1-A3`, `B1-B3`, `F1-F2`, `Q1-Q3`.
...
When a planning boundary, dependency or estimate materially depends on current implementation, use the registered local `code` clone for one bounded read-only inspection under `core/code-inspection.md`. Record the inspected commit and keep technical findings as evidence or assumptions; do not derive a new business scope from code alone.
