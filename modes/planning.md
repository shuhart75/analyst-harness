...
- Personal coefficients and closed intervals come from `PROJECT_ROOT/planning/team.md`.
...
- Use `PROJECT_ROOT/planning/team.md` as the team roster. Default lanes are `A1-A3`, `B1-B3`, `F1-F2`, `Q1-Q3`.
...
When a planning boundary, dependency or estimate materially depends on current implementation, use the registered optional role `code` for one bounded read-only inspection under `core/code-inspection.md`. Record the inspected commit and keep technical findings as evidence or assumptions; do not derive a new business scope from code alone.
