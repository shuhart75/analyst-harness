...
При адаптивной актуализации следуй `core/tracker-adaptive.md`: выбранный трекер
без автоматического второго, обязательная проверка истории до предложения дат,
одна QA-работа на фичу с суммой QA-оценок уникальных карточек либо подтверждённые
непересекающиеся релизные QA-группы по `core/tracker-release.md`. Подтверждённые
удаления входят в просмотренный diff текущего исполнения; планы и архивы неизменны.
...
Прогресс общего QA — доля прошедших тестирование среди всех неотменённых FE/BE-задач
фичи, независимо от наличия оценки QA в карточках. Сохраняй числитель, знаменатель
и основание; неполный состав не доказывает 100%. Оценку QA повторно не суммируй.
...
- Завершённая реальная BE/FE-задача с 100% и подтверждённым `Actual Finish` либо `Completed By` может иметь неизвестную оценку (`-`): показывай факт без выдуманного веса или длительности. При неизвестном начале точное окончание показывается отдельной отметкой, не интервалом; начало из плана не подставляется, ресурс не резервируется. Колонка Completed By для этого не требуется; точный Actual Finish не подменяй границей. Для прогнозного размещения положительная оценка обязательна; QA-разбиение также требует подтверждённой общей оценки. При неизвестном весе одной задачи общий взвешенный прогресс неизвестен, кроме случая, когда все учитываемые задачи завершены на 100%.
- `Progress %` допускает число от 0 до 100 с дробной частью через точку либо `unknown`. Реестр сохраняет согласованное значение (например, `85.7`); целочисленное заполнение полосы PlantUML округляется, дробный процент остаётся в подписи задачи.
...
- Not-started tasks must be capacity-scheduled by `PROJECT_ROOT/planning/team.md`: no resource lane above 100% on an open workday, and available resources should be used before pushing work later.
...
- collect current planning stories, `actualization.md`, execution tasks and `PROJECT_ROOT/planning/team.md`;
...
- Prefer canonical executor/resource lanes from `PROJECT_ROOT/planning/team.md`: `A1-A3`, `B1-B3`, `F1-F2`, `Q1-Q3`.
