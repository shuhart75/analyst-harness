...
- Завершённая реальная BE/FE-задача с 100% и подтверждённым интервалом либо `Completed By` может иметь неизвестную оценку (`-`): показывай факт без выдуманного веса или длительности. Для прогнозного размещения положительная оценка обязательна; QA-разбиение также требует подтверждённой общей оценки. При неизвестном весе одной задачи общий взвешенный прогресс неизвестен, кроме случая, когда все учитываемые задачи завершены на 100%.
