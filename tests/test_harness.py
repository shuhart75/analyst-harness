...
class HarnessTests(unittest.TestCase):
    ...
    def test_scaffold_contains_requirements_profile(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("делегирование чтения", tracker_contract)
            self.assertIn('"status": "tracker-read-reconciled"', tracker_contract)
            self.assertIn("active-inventory-v2", tracker_contract)
            self.assertIn("tracker-session-log.md", tracker_contract)
            self.assertIn("mcp-log", tracker_contract)
            self.assertIn("inventory-page", tracker_contract)
            self.assertIn("selection-complete", tracker_contract)
            self.assertIn("history-complete", tracker_contract)
            self.assertIn("Не требуется читать и записывать подробную карточку каждой задачи", tracker_contract)
            self.assertIn("`SberTrek.Объект Jira == Jira.key`", tracker_contract)
            self.assertIn("Собственные ключи задач могут совпадать или различаться", tracker_contract)
            self.assertIn("--team-id B2", tracker_contract)
            self.assertIn("Один `team_id` не может обозначать несколько account id", tracker_contract)
            self.assertIn("run-status", tracker_contract)
            self.assertIn("result-status", tracker_contract)
            self.assertIn("поэлементное правило", tracker_contract)
            self.assertIn("1 SP = 1 человекодень", tracker_contract)
            self.assertIn("Прямые ответы MCP не являются отчётом", tracker_contract)
            ...
            self.assertIn("`active-inventory-v2`", readme)
            self.assertIn("`tracker-session-log.md`", readme)
            self.assertIn("Подробные карточки и история читаются только для выбранных", readme)
            self.assertIn("`SberTrek.Объект Jira == Jira.key`", readme)
            self.assertIn("официальный статус", readme)
            self.assertIn("1 SP = 1 человекодень", readme)
            self.assertIn("Старые schema-8/schema-9 сеансы", readme)
