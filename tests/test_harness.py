...
class HarnessTests(unittest.TestCase):
    ...
    def test_scaffold_contains_requirements_profile(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("Все вызовы\nвыполняет основной агент", tracker_contract)
            self.assertIn('"status": "tracker-read-reconciled"', tracker_contract)
            self.assertIn("targeted-tracker-v1", tracker_contract)
            self.assertIn("tracker-session-log.md", tracker_contract)
            self.assertIn("mcp-log", tracker_contract)
            self.assertIn("query-page", tracker_contract)
            self.assertIn("collection-advance", tracker_contract)
            self.assertIn("history-complete", tracker_contract)
            self.assertIn("Карточка обязательна для каждого возвращённого ключа", tracker_contract)
            self.assertIn("SberTrek.issue_key == Jira.key", tracker_contract)
            self.assertIn("собственному ключу SberTrek", tracker_contract)
            self.assertIn("`AN1`, `BE2`, `FE1`, `QA1`", tracker_contract)
            self.assertIn("Один `team_id` нельзя назначить двум аккаунтам", tracker_contract)
            self.assertIn("run-status", tracker_contract)
            self.assertIn("result-status", tracker_contract)
            self.assertIn("Для каждого поля", tracker_contract)
            self.assertIn("1 SP = 1 человекодень", tracker_contract)
            self.assertIn("Прямой ответ MCP не является отчётом", tracker_contract)
            self.assertIn("пересказывать полученные данные вручную", tracker_contract)
            ...
            self.assertIn("`targeted-tracker-v1`", readme)
            self.assertIn("tracker-session-log.md", readme)
            self.assertIn("Карточка обязательна для каждого возвращённого ключа", readme)
            ...
            self.assertIn("Старые schema-8/schema-9", readme)
