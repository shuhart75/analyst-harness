...
class HarnessTests(unittest.TestCase):
    ...
    def test_scaffold_contains_requirements_profile(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("JSON схемы 7", tracker_contract)
            ...
            self.assertIn("точное равенство ключа", tracker_contract)
            self.assertIn("`issue_key` сохраняет только справочный смысл", tracker_contract)
