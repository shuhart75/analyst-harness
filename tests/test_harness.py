...
class HarnessTests(unittest.TestCase):
    ...
    def test_scaffold_contains_requirements_profile(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("изолированный сборщик", tracker_contract)
            self.assertIn('"status": "tracker-read-reconciled"', tracker_contract)
            self.assertIn("targeted-tracker-v3", tracker_contract)
            self.assertIn("tracker-session-log.md", tracker_contract)
            self.assertIn("collector-brief", tracker_contract)
            self.assertIn("history-job", tracker_contract)
            self.assertIn("не более восьми ключей", tracker_contract)
            self.assertIn("описания, комментарии, вложения", tracker_contract)
            self.assertIn("SberTrek.issue_key == Jira.key", tracker_contract)
            self.assertIn("`AN1`, `BE2`, `FE1`, `QA1`", tracker_contract)
            self.assertIn("run-status", tracker_contract)
            self.assertIn("result-status", tracker_contract)
            self.assertIn("--history-item-count", tracker_contract)
            self.assertIn("`response_contract.text` дословно", tracker_contract)
            self.assertIn("не запускает `--help`", tracker_contract)
            self.assertIn("1 SP = 1 человекодень", tracker_contract)
            delegation = (ROOT / "core/agent-delegation.md").read_text(encoding="utf-8")
            self.assertIn("fresh isolated subagent", delegation)
            ...
            self.assertIn("fresh isolated subagent", llm_contract)
            self.assertIn("The coordinator must never call tracker MCP", llm_contract)
            self.assertIn("issue.exportJson", llm_contract)
            self.assertIn("issue.search.text", llm_contract)
            self.assertIn("max_results: 50", llm_contract)
            self.assertIn("Several provider account IDs may map to one `team_id`", llm_contract)
            self.assertIn("never runs `begin`", llm_contract)
            self.assertIn("`counts`, `summary` and `limitations`", llm_contract)
            ...
            self.assertIn("delegation_required", (ROOT / "scripts/trackerctl.py").read_text(encoding="utf-8"))
            self.assertIn("must not call tracker MCP itself", agent_rules)
            ...
            self.assertIn("Emit the returned `response_contract.text` verbatim and nothing else", agent_rules)
            self.assertIn("never run `trackerctl.py --help`", agent_rules)
            self.assertIn("1 SP = 1 person-day", agent_rules)
            readme = (ROOT / "README.md").read_text(encoding="utf-8")
            self.assertIn("код `3` и `must_stop: true` запрещают MCP-вызовы", readme)
            self.assertIn("конечные и исключающие статусы", readme)
            self.assertIn("`targeted-tracker-v3`", readme)
            self.assertIn("tracker-session-log.md", readme)
            self.assertIn("detail-вызовы запрещены", readme)
            self.assertIn("`issue.exportJson`", readme)
            self.assertIn("`max_results: 50`", readme)
            self.assertIn("Несколько\n   account id одного человека", readme)
            self.assertIn("повторный `begin` блокируется", readme)
            self.assertIn("`counts`, `summary` и `limitations`", readme)
            self.assertIn("`SberTrek.issue_key == Jira.key`", readme)
            self.assertIn("официальный статус", readme)
            self.assertIn("1 SP = 1 человекодень", readme)
            self.assertIn("schema-8/schema-9", readme)
