...
class HarnessTests(unittest.TestCase):
    ...
    def test_scaffold_contains_requirements_profile(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("direct-tracker-v1", tracker_contract)
            self.assertIn("Основной агент сам выполняет каждый MCP-вызов", tracker_contract)
            self.assertIn("next_action", tracker_contract)
            self.assertIn("step_id", tracker_contract)
            self.assertIn("ingest-error", tracker_contract)
            self.assertIn("history-not-collected", tracker_contract)
            ...
            self.assertNotIn("targeted-tracker-v3", tracker_contract)
            delegation = (ROOT / "core/agent-delegation.md").read_text(encoding="utf-8")
            self.assertIn("Tracker work is never delegated", delegation)
            llm_contract = (ROOT / "core/llm-contract.md").read_text(encoding="utf-8")
            self.assertIn("direct-tracker-v1", llm_contract)
            self.assertIn("The main agent itself executes the exact MCP call", llm_contract)
            ...
            self.assertIn("history-not-collected", llm_contract)
            ...
            self.assertIn("Tracker work is never delegated", agent_rules)
            self.assertIn("direct-tracker-v1", agent_rules)
            workflow = (ROOT / "scripts/tracker_workflow.py").read_text(encoding="utf-8")
            self.assertIn('PROTOCOL = "direct-tracker-v1"', workflow)
            self.assertNotIn("delegation_required", workflow)
            readme = (ROOT / "README.md").read_text(encoding="utf-8")
            self.assertIn("`direct-tracker-v1`", readme)
            self.assertIn("основной агент сам выполняет указанный MCP-вызов", readme)
            ...
            self.assertNotIn("`targeted-tracker-v3`", readme)
            active_tracker_docs = "\n".join((
                tracker_contract,
                llm_contract,
                agent_rules,
                readme,
                (ROOT / "core/tooling-policy.md").read_text(encoding="utf-8"),
                (ROOT / "templates/workflow/command-cheatsheet.template.md").read_text(encoding="utf-8"),
            ))
            for obsolete in ("collector-brief", "history-job", "tracker-session-log.md", "targeted-tracker-v3"):
                self.assertNotIn(obsolete, active_tracker_docs)
