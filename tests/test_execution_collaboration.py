...
class ExecutionCollaborationTests(unittest.TestCase):
    prepare_workspace = test_collaboration.CollaborationTests.prepare_workspace
    git = test_collaboration.CollaborationTests.git
    collaboration = test_collaboration.CollaborationTests.collaboration
    ...
    def raw(self, *args):
        return test_collaboration.run(sys.executable, str(test_collaboration.COLLABORATION),
    ...
    def test_one_commit_contains_multiple_sources_and_complete_quarter_views(self):
        ...
        result = test_collaboration.run(*command, env=self.environment)
