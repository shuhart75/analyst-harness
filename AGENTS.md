...
- After `begin`, use `active-inventory-v2`. Immediately after every real MCP call and before transforming its response, record logical provider, operation, outcome, evidence and a bounded summary through `mcp-log`; never log a planned call, credentials, MCP server names or complete response bodies. Then read paginated unfinished-task inventories for configured SberTrek projects and Jira projects, recording one `inventory-page` evidence item per real MCP page and every returned SberTrek `Объект Jira` link. Select by known key, explicit link, title/description or epic context and ask one exact question for each ambiguous candidate. Do not direct-read every inventory key. Record details only for selected or reviewed candidates; a page containing a full card is valid detail evidence. Read history only for selected relevant tasks, complete explicit-link closure, finalize snapshots, run `run-status`, then `reconcile`. Pair only when `SberTrek.Объект Jira == Jira.key`; SberTrek is primary, Jira fills missing fields, and populated conflicts preserve SberTrek plus an explicit discrepancy. Direct MCP data is not a report. Preserve the generated `tracker-session-log.md` beside the other run files.
...
- Start with two bounded active inventories: unfinished configured-project tasks from SberTrek, then Jira. Record every returned key but do not direct-read every task. Starting from known real keys shaped like `PROJECT-123`, select only known tasks, explicit counterparts and title/description/epic candidates. A text-search hit is not a known key. Ask one exact question for an ambiguous candidate. Report releases missing from actual-progress as proposals only.
- Run the exact `config-status -> begin -> (MCP -> mcp-log -> structured record) -> selection-complete -> history-complete -> snapshot-finalize -> run-status -> reconcile -> result-status` protocol. Ask every returned configuration, relevance or participant question one at a time. For a participant, ask only for the roster `team_id`; normalize `A/B/F/Q` aliases to `AN/BE/FE/QA` and derive the role from that prefix. Never invent mappings, reuse one `team_id` for two accounts of the same provider, continue after a guard error or reconcile incomplete inventories.
...
- Record detailed cards and history only for selected relevant tasks. A full card on an inventory page is valid detail evidence; otherwise use one exact-key call for that selected task. Explicitly distinguish a returned value, confirmed absence and a field not returned for `assignee`, `estimate`, `epic` and `releases`. Unknown accounts from irrelevant inventory tasks must not enter the `team_id` queue.
