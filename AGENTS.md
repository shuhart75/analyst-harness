...
- Each collector reads only `core/tracker-collector.md` and its job JSON. It executes the job's exact TQL/JQL, never reads MCP documentation, never probes an alternative query and never fetches returned cards one by one. It records compact cards without descriptions/comments/full bodies, then runs the job completion command and returns only status and paths. The coordinator reads only `run-status` between jobs. `trackerctl.py` alone derives counterpart queries, splits history into bounded batches, pairs `SberTrek.issue_key == Jira.key`, preserves SberTrek field priority and writes `reconciled.json`.
