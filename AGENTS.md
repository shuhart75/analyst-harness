...
- After `begin`, run and record an independent bounded `feature_search` for every enabled provider; one provider's search never substitutes for the other's. Put every returned key in that provider's `feature_search.checked_keys`, then direct-read and register every hit even when it is ambiguous or irrelevant. Persist every MCP result immediately through `snapshot-metadata`, `snapshot-issue`, `snapshot-history`, `snapshot-not-found` and `snapshot-collection`; do not postpone normalization or hand-author the complete snapshot. Record an issue only after a direct exact-key read and pass one provider-prefixed call containing that exact key as `snapshot-issue --evidence`; an exact-key search returning the full card is valid, a feature-search hit alone is not. Every evidence element must describe exactly one MCP call and contain exactly one `mcp:` prefix; do not concatenate calls or use `:none`. For every issue, classify `assignee`, `estimate`, `epic` and `releases` as an actual value, confirmed absence or a field not returned by MCP. Normalize SberTrek `Attributes.assigned_to` to `assignee` and `Attributes.story_points` to `estimate`; do not mark a field `not-returned` merely because it is nested or provider-specific. Complete `history`, `epic_links` and `release_links` claims must have one evidence containing each exact issue key. Complete `epic_neighbors.checked_keys` and `expanded_epic_keys` must equal the actual discovered epic keys and must never contain ordinary task keys; when none exist, complete this capability without evidence so trackerctl records the machine-derived empty set. Expected epic and release keys are provider-specific and block a complete snapshot when unresolved. After all provider calls, run `snapshot-finalize`; it creates `captured_at` itself and makes the snapshot immutable. Issue and history timestamps cannot be later. Every other ordinary `complete` or `unavailable` collection claim requires provider-prefixed `--evidence` from an actual MCP call; `capability-absent` specifically requires evidence from MCP capability discovery and cannot be inferred from an issue read. `cross_provider_lookup=complete` computes evidence and checked keys automatically and rejects manual values. A skipped call is never `unavailable`. Run `run-status --run-id <run_id>`, complete every reported gap, then run the exact `reconcile --run-id <run_id>` command. Direct MCP data is not a report.
- A tracker summary is allowed only after `trackerctl.py reconcile` returns `status: tracker-read-reconciled`, `workflow_complete: true` and `final_response_allowed: true` for that `run_id`. After successful reconciliation, obtain machine status only through `result-status --run-id <run_id>` or `run-status --run-id <run_id>`; both must return the persisted official completion envelope. Never assemble a completion status manually. Any nonzero exit or `final_response_allowed: false` forbids a user-facing summary even when MCP data has already been read; perform only `allowed_next_action` and retry the guard.
...
- Merge paired `epic`, `assignee` and `estimate` fields independently: prefer a populated SberTrek value and use Jira only when that SberTrek field is missing or not returned. Preserve a populated SberTrek value on conflict and report the discrepancy. Normalize `SP`, story points and person-days to `story-points` using the agreed `1 SP = 1 person-day`; never convert other units silently.
...
- Starting from known real task keys shaped like `PROJECT-123`, record one analytical `seed_evidence` source per seed, then inspect the other tasks in each discovered epic as bounded candidates. When there are no known keys, keep the seed list empty; never invent placeholders such as `FEATURE`. A text-search hit is not a seed. Analyze candidate title and description; propose directly evidenced matches and ask about ambiguous matches. Report releases missing from actual-progress as proposals only.
- Run the exact `config-status -> begin -> reconcile` protocol from `core/tracker-reading.md`. Ask every returned configuration or participant question one at a time until the returned participant queue is empty. For a participant, ask only for the roster `team_id`; normalize `A/B/F/Q` aliases to `AN/BE/FE/QA` and derive the role from that prefix. Never invent a mapping, reuse one `team_id` for two accounts of the same provider, continue after a guard error, mark a collection capability complete without calling it, or reconcile a default/empty configuration.
- Pair SberTrek and Jira issues only when their keys are exactly equal. Never use `issue_key`, `counterpart_key`, titles, descriptions, assignees, epics, releases or manual mappings for pairing; such link fields are informational only.
- Direct-read every key discovered in either tracker under that exact key in both trackers. Record a confirmed absence through evidenced `snapshot-not-found`; direct-read and not-found evidence must use the logical provider prefix `mcp:sbertrek:` or `mcp:jira:` matching the snapshot. Without a found issue or exact not-found result, the cross-provider check remains incomplete. Preserve provider timestamps exactly. A search result alone does not prove complete history, epic, release or cross-provider collection.
- For every recorded issue, explicitly distinguish a returned value, confirmed absence and a field not returned by MCP for `assignee`, `estimate`, `epic` and `releases`. Never omit the assignee silently: a returned account on a configured development type must enter the sequential `team_id` queue. `history=complete` must name every checked issue key.
