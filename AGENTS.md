...
- After `begin`, persist every MCP result immediately through `snapshot-metadata`, `snapshot-issue`, `snapshot-history`, `snapshot-not-found` and `snapshot-collection`; do not postpone normalization or hand-author the complete snapshot. For every issue, classify `assignee`, `estimate`, `epic` and `releases` as an actual value, confirmed absence or a field not returned by MCP. A `history=complete` claim must list every issue key actually checked. Every `complete` or `unavailable` collection claim requires `--evidence` from an actual MCP call; a skipped call is never `unavailable`. Run `run-status --run-id <run_id>`, complete every reported gap, then run the exact `reconcile --run-id <run_id>` command. Direct MCP data is not a report.
