...
- Each collector reads only `core/tracker-collector.md` and its job JSON. It executes the job's exact TQL/JQL, never reads MCP documentation, never probes an alternative query and never fetches returned cards one by one. For SberTrek it requests the job's preferred projection instead of `fields=null`, never treats a rendered or truncated preview as data, and passes the complete MCP-produced JSON file to `trackerctl.py ingest-query-response`; only that command may count and record SberTrek cards. It records no descriptions/comments/full bodies, then runs the job completion command and returns only status and paths. The coordinator reads only `run-status` between jobs. `trackerctl.py` alone derives counterpart queries, splits history into bounded batches, pairs `SberTrek.issue_key == Jira.key`, preserves SberTrek field priority and writes `reconciled.json`.
