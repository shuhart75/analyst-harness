...
- After `begin`, use `active-inventory-v1`: read paginated unfinished-task inventories for configured SberTrek projects and then Jira projects, recording one `inventory-page` evidence item per real MCP page and every returned SberTrek `Объект Jira` link. Select by known key, explicit link, title/description or epic context and ask one exact question for each ambiguous candidate. Do not direct-read every inventory key. Record details only for selected or reviewed candidates; a page containing a full card is valid detail evidence. Read history only for selected relevant tasks, complete explicit-link closure, finalize snapshots, run `run-status`, then `reconcile`. Pair only when `SberTrek.Объект Jira == Jira.key`; SberTrek is primary, Jira fills missing fields, and populated conflicts preserve SberTrek plus an explicit discrepancy. Direct MCP data is not a report.
