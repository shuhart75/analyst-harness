...
- After `begin`, use `targeted-tracker-v1`. The scope must be either one epic or an explicit set of task keys in an explicitly known provider; Jira and SberTrek keys have the same shape, so ask one question when the provider is ambiguous. Execute only the exact TQL/JQL returned as `next_query`. Immediately after every real MCP call and before transforming its response, record provider, operation, outcome, evidence, exact query when applicable and a bounded summary through `mcp-log`; never log a planned call, credentials, MCP server names or complete response bodies. Record every returned key through `query-page` and a full structured card through `record-issue`. Run `collection-advance` to obtain the single exact counterpart query, then record its complete result. Never run a global project inventory, free-text search or semantic candidate search. Read history only after collection completion, finalize snapshots, run `run-status`, then `reconcile`. Pair only when the SberTrek card's `issue_key` equals `Jira.key`; SberTrek is primary, Jira fills missing fields, and populated conflicts preserve SberTrek plus an explicit discrepancy. Direct MCP data is not a report. Preserve `tracker-session-log.md` beside the other run files.
...
- Run the exact `config-status -> begin -> (exact MCP query -> mcp-log -> query-page -> record-issue) -> collection-advance -> (exact counterpart query -> mcp-log -> query-page -> record-issue) -> collection-advance -> history-complete -> snapshot-finalize -> run-status -> reconcile -> result-status` protocol. Ask every returned configuration or participant question one at a time. For a participant, ask only for the roster `team_id`; normalize `A/B/F/Q` aliases to `AN/BE/FE/QA` and derive the role from that prefix. Never invent mappings, reuse one `team_id` for two accounts of the same provider, continue after a guard error or reconcile an incomplete collection.
...
- Record a detailed card for every key returned by each exact target query. A full card on a query page is valid detail evidence; otherwise use one exact-key call for that task. Explicitly distinguish a returned value, confirmed absence and a field not returned for `assignee`, `estimate`, `epic` and `releases`.
