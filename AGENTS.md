...
- Collection import never receives a model-selected `--page-number` or model-selected evidence: `trackerctl.py` derives the next ordinal and evidence from the provider, query stage, exact query and full-response SHA-256. Jira `start_at` is only a continuation cursor, while the single non-paginated SberTrek export is recorded internally as page 1. A successful collection call is never pre-logged through `mcp-log`. Re-importing the identical full response is a successful no-op; a different response for an already occupied step remains an integrity failure. Counterpart-epic discovery and member export always receive distinct machine evidence.
- A premature `reconcile` never destroys usable collection state. While a collection job is pending or running it returns that job as the next action without failing the run. When collection is complete, `reconcile` may explicitly skip remaining history jobs, records per-issue `*-history-not-collected:*` limitations and continues from current fields; unproven lifecycle facts remain `unknown`. Wrong scope/query, malformed JSON, conflicting repeated responses and direct runtime-artifact edits remain terminal integrity failures.
