...
def run_branch_workflow(root: Path, command: str) -> dict:
    result = run(
        sys.executable,
        str(Path(__file__).with_name("branch-workflow.py")),
        "--root",
        str(root),
        command,
    )
...
def migrate_command(args: argparse.Namespace) -> int:
    ...
    if active_merge(analytics):
        raise ValueError(
            "В analytics уже выполняется слияние. Разреши каждый конфликтующий путь отдельным "
            "решением и заверши либо осознанно отмени именно это слияние"
        )
    ...
    main_update = run_branch_workflow(root, "fast-forward-main") if relation == "behind" else None
    write_state(root, state)
    print(json.dumps({
...
def update_command(args: argparse.Namespace) -> int:
    ...
    work = require_active_work(root, analytics, state)
    require_clean(analytics)
    updated = run_branch_workflow(root, "update-feature")
    pushed = git(analytics, "push", "--set-upstream", "origin", f"HEAD:{work['branch']}")
    if pushed.returncode != 0:
...
def submit_command(args: argparse.Namespace) -> int:
    ...
    print(json.dumps({
        ...
        "message": (
            ...
            "человеку нужно создать и принять его в analytics/main."
        ),
...
def finish_command(args: argparse.Namespace) -> int:
    ...
    if not is_ancestor(analytics, work_commit, remote):
        print(json.dumps({
            ...
            "origin_main": remote,
            "message": "Рабочая ветка ещё не принята в analytics/main; передачу создавать нельзя",
        }, ensure_ascii=False, indent=2))
        return 2
    if branch != BRANCH:
    ...
    main_update = run_branch_workflow(root, "fast-forward-main")
    completed = {
        **work,
        "status": "merged",
        "merged_at": utc_now(),
    ...
    print(json.dumps({
        ...
        "current_branch": BRANCH,
        "main_update": main_update,
        "next_action": "разрешено начать аудит перед передачей требований из актуальной main",
    }, ensure_ascii=False, indent=2))
    return 0
...
def require_main_for_delivery_command(args: argparse.Namespace) -> int:
    ...
    if state is None:
        print(json.dumps({
            ...
            "message": (
                ...
                "аудит для передачи и редакцию передачи создавать нельзя."
            ),
        }, ensure_ascii=False, indent=2))
        return 2
    if state.get("active_work"):
    ...
    if current_branch(analytics) != BRANCH:
        raise ValueError("Передачу в разработку разрешено формировать только из main")
    remote = fetch_main(analytics)
    local = head(analytics)
    if local != remote:
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(description="Совместная работа аналитиков в PROJECT_ROOT")
