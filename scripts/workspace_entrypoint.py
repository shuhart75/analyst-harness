...
def write_local_entrypoint(project: Path, harness: Path, code: Path | None = None) -> Path:
    ...
    path.write_text(
        ...
        "Для истории, статуса и diff проекта используй git -C PROJECT_ROOT. Игнорирование каталога проекта "
        "в HARNESS_ROOT не означает отсутствие его собственного Git; не меняй .gitignore и не выполняй git init.\n"
        "Для фактического прогресса сначала прочитай modes/execution-update.md в HARNESS_ROOT. "
        "Регенерируй только с --actual-only; при неполных источниках остановись без удаления старого Ганта, "
        "создания фиктивных срезов или вымышленных ключей трекера.\n"
        ...
        "и программы HARNESS_ROOT/scripts/code-inspect.py. Сначала workspace.py project-root, затем "
        "code-inspect.py route PROJECT_ROOT: используй возвращённый абсолютный root, выбери один контур "
        "и выполни begin, locate, чтение и verify. Все программы запускай из HARNESS_ROOT/scripts. "
        "Путь к code не спрашивай и не угадывай. Поиск в обвязке не доказывает отсутствие кода. "
        "Пустой поиск относится только к запросу, контуру и коммиту. Отвечай по-русски с коммитом и путями. "
        "Разовый вопрос по коду не требует старта feature-работы.\n"
