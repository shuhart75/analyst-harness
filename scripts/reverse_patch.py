...
def require_collaboration_ready_for_apply(root: Path) -> None:
    ...
    if not collaboration_path.is_file():
        raise ValueError(
            "Совместная работа ещё не настроена; reverse patch нельзя применять до одноразовой migration"
        )
    ...
    if active:
        ...
        raise ValueError(
            f"Reverse patch нельзя применять при активной feature-сессии {branch}; "
