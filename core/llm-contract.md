...
- After `begin`, record metadata, each direct exact-key issue read, each not-found direct read, each history event and each collection capability immediately through the matching `snapshot-*` command. `snapshot-issue` requires evidence from the exact-key read; a search result must be direct-read before recording. For each issue, explicitly classify `assignee`, `estimate`, `epic` and `releases` as `value`, `absent` or `not-returned`; never turn a field omitted by MCP into confirmed absence. Seeds must be real `PROJECT-123` keys; when none are known, keep the list empty instead of inventing a placeholder. Every seed must become a same-provider issue or an evidenced `snapshot-not-found` record. Ordinary `complete` and all `unavailable` collection claims require evidence from an actual MCP call; `cross_provider_lookup=complete` derives it automatically, while `history=complete` additionally lists every checked issue key. Never label a skipped call unavailable. Use `run-status` to find gaps. Do not replace this recording step with a manually assembled table or a large hand-written snapshot.
...
- Pair tracker issues only by exact key equality. For every key discovered in either tracker, direct-read that exact key in both providers and record a found issue or evidenced `snapshot-not-found`; `issue_key`, `counterpart_key`, text and manual mappings never affect pairing.
- Do not proceed with an incomplete configuration, a pending collection capability, an unresolved seed, an unchecked exact key or an unknown participant. `set-participant` is valid only for the exact pending participant and `run_id`; ask all returned participant questions sequentially and accept only a roster `team_id`, never an invented role.
