...
- Receiver-side context is disclosed progressively: active `handoff.json`, compact request and manifest, one relevant contour of packaged requirements, that contour's local SDD, matched code, then nearby tests. Do not load the whole code repository or multiple contours without a recorded cross-contour dependency.
- Analyst-side code access is also progressive and read-only. Resolve the configured code repository through `.workflow/code-repos.json`, snapshot it with `code-inspect.py begin`, inspect one contour and bounded matches, then prove no code-repository changes with `code-inspect.py verify`.
...
- When current implementation facts affect requirements and code access is configured, inspect the registered repository automatically under `.workflow/code-inspection.md`. Record the exact commit and relative evidence paths; do not infer business intent from code alone. Without code access, record the unchecked assumption and continue.
