...
- Pair only when `SberTrek.issue_key == Jira.key`. For epic, assignee, general estimate and each `AN / BE / FE / QA` estimate, use populated SberTrek first and Jira only for a gap; expose differing populated values as conflicts. `1 SP = 1 person-day`; do not silently convert other units.
...
- `reconcile` only saves the result and returns `next_action.type=result-status`; its response never authorizes a user-facing summary. Run the returned command and present task facts only when `result-status` returns `tracker-read-reconciled`, `workflow_complete: true` and `final_response_allowed: true`. Emit its checksum-validated `response_contract.text` verbatim, without reading result files or composing another summary.
