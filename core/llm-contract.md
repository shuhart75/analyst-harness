...
- Infer a general estimate as `AN`, `BE` or `FE` only when there are no role estimates and the title has one unambiguous role prefix. Every role estimate creates its own `<tracker-key>/<role>` work item and Gantt bar. The overall estimate sums those role work items and only general estimates that remain unresolved; it never counts a decomposed general estimate twice. Strip role brackets from its label: `[FE] Name` becomes `FE Name`.
