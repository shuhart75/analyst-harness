...
- Pair only when `SberTrek.issue_key == Jira.key`. Jira fills gaps in assignee, general estimate and each `AN / BE / FE / QA` estimate. Different populated values stop reconciliation on one task: ask only the returned question, then record SberTrek, Jira or a custom per-field decision. A provider decision may explicitly apply to all following conflicts in the current run only. Equal normalized assignee names with different provider IDs are not conflicts; linked-epic keys, provider status dictionaries and service dates do not require analyst resolution. `1 SP = 1 person-day`; do not silently convert other units.
- Infer a general estimate as `AN`, `BE` or `FE` only when there are no role estimates and the title has one unambiguous role prefix. Every role estimate creates its own `<tracker-key>/<role>` work item and Gantt bar. Strip role brackets from its label: `[FE] Name` becomes `FE Name`.
