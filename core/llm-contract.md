...
- On exit code `3` with `must_stop: true`, perform only `allowed_next_action: ask-user`: emit exactly `response_contract.text`, without a preface, explanation, examples or suggested answers. Do not call tracker MCP tools, search for task facts, create a task list, delegate work or present a tracker summary.
...
- After `begin`, record metadata, each issue, each not-found direct read, each history event and each collection capability immediately through the matching `snapshot-*` command. For each issue, explicitly classify `assignee`, `estimate`, `epic` and `releases` as `value`, `absent` or `not-returned`; never turn a field omitted by MCP into confirmed absence. Every seed must become a same-provider issue or an evidenced `snapshot-not-found` record. Every `complete` or `unavailable` collection claim requires evidence from an actual MCP call; `history=complete` additionally lists every checked issue key. Never label a skipped call unavailable. Use `run-status` to find gaps. Do not replace this recording step with a manually assembled table or a large hand-written snapshot.
