...
- Pass the full MCP JSON to `ingest` using the returned `step_id`. Prefer an MCP-created file; a complete inline response may be captured verbatim outside the run. Never extract, summarize, reconstruct or store the raw response inside the run. An identical import is a no-op; a different response for a completed step, malformed JSON or a stale step is fatal.
