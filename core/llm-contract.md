...
- After `begin`, record metadata, each issue, each read history event and each collection capability immediately through the matching `snapshot-*` command. Use `run-status` to find gaps. Do not replace this recording step with a manually assembled table or a large hand-written snapshot.
