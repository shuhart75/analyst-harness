...
- Build one paginated unfinished-task inventory per enabled provider, SberTrek first. Select known keys, explicit counterparts and title/description/epic candidates from those inventories. Surface ambiguous membership to the analyst one question at a time. Missing releases are proposals for later actual-progress application, not read-time writes.
...
- After every real tracker MCP call, immediately record its logical provider, operation, outcome, evidence and bounded result summary through `mcp-log`; do this before transforming the response. Never log a planned or invented call, MCP server names, credentials or complete response bodies. `inventory-page`, `inventory-unavailable`, `record-issue` and `history-complete` accept only previously logged evidence. The automatically maintained `tracker-session-log.md` must remain beside the other run files.
- After `begin`, record every real inventory page through `inventory-page`, including every returned key and every SberTrek `Объект Jira` value. Do not direct-read every inventory key. Use `record-issue` only for selected or reviewed candidates; a page returning a full card is valid detail evidence, otherwise use one exact-key detail call for that selected task. Read and record history only for selected relevant tasks. For each recorded issue, classify `assignee`, `estimate`, `epic` and `releases` as `value`, `absent` or `not-returned`; never turn an omitted field into confirmed absence. Use `run-status` and `tracker-session-log.md` to find gaps and never replace machine state with a hand-written snapshot.
- Start tracker MCP calls only after a ready configuration and successful `begin` returning a `run_id`. Do not delegate tracker reading. Present task facts only after `reconcile` returns `tracker-read-reconciled`, `workflow_complete: true` and `final_response_allowed: true` for that same `run_id`. Retrieve later machine status only with `result-status` or `run-status`, which return the persisted successful envelope; never synthesize its flags or counts. A failed guard forbids a manually assembled fallback summary.
- Pair tracker issues only when the SberTrek card's returned `Объект Jira` equals `Jira.key`. Equality of own keys, text, epics, releases and manual mappings never creates a pair. Close both sides of every selected active explicit link.
- Do not proceed with an incomplete configuration, unfinished inventory pagination, unresolved ambiguous relevance, incomplete selected-task history, open link closure or unknown selected-task participant. `set-participant` is valid only for the exact pending participant and `run_id`; ask all returned participant questions sequentially and accept only a roster `team_id`, never an invented role.
