...
- After every real tracker MCP call, the isolated collector immediately records provider, operation, outcome, evidence, exact query when applicable and a bounded summary. Jira collection and both history providers use `mcp-log`. SberTrek collection must use `issue.exportJson` or a semantically equivalent bulk JSON export operation: pass the exact TQL through its `query` parameter, never through `issue.search.text`, and never substitute `issue.getByKey` or `link.list`. A successful SberTrek export passes the complete MCP-produced JSON file directly to `ingest-query-response`, which atomically logs evidence, computes SHA-256 and the item count, and records every compact card. Never inspect a truncated preview, supply `fields=null` when projection is available, create a shortened response copy, log a planned call, MCP server names, credentials or complete response bodies. Collection jobs preserve compact cards only; history jobs preserve only `assignee` and `status` events.
