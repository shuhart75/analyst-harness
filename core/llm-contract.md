...
- Receiver-side context is disclosed progressively: active `handoff.json`, compact request and manifest, one relevant contour of packaged requirements, that contour's local SDD, matched code, then nearby tests. Do not load all of `code` or both contours without a recorded cross-contour dependency.
- Analyst-side code access is also progressive and read-only. Resolve the sibling `code` clone through `templates/workflow/code-repos.template.json`, snapshot it with `code-inspect.py begin`, inspect one contour and bounded matches, then prove no code-repository changes with `code-inspect.py verify`.
...
- Not-started execution tasks must not overload resource lanes. Use `planning/team.md` as the roster, keep each resource at no more than one full-time task per open day, and use available resources as fully as possible before pushing work later.
...
- The project-local roster lives in `planning/team.md`.
...
- When current implementation facts affect requirements, inspect the registered local `code` clone automatically under `core/code-inspection.md`. Record the exact commit and relative evidence paths; do not infer business intent from code alone.
...
3. update `features/<feature>/domain-impact.md` and `planning/consistency-backlog.md` if the change affects consistency tracking;
...
- update `planning/consistency-backlog.md` for any impact not propagated immediately.
...
Prototype updates are optional unless the prototype is an active scope-demo or delivery-handoff artifact. Still, affected prototypes must be listed in `domain-impact.md` and/or `planning/consistency-backlog.md` so the user can later say "актуализируй прототипы" and the agent has a concrete target list.
