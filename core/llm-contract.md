...
- Reduce the request to one explicit epic or an explicit set of task keys in a known provider. Ask one question when the provider or scope is ambiguous. Use only the exact targeted TQL/JQL returned by `trackerctl`; never build a full-project inventory or use title/description search. Missing releases are proposals for later actual-progress application, not read-time writes.
...
- After every real tracker MCP call, immediately record its logical provider, operation, outcome, evidence, exact query when applicable and bounded result summary through `mcp-log`; do this before transforming the response. Never log a planned or invented call, MCP server names, credentials or complete response bodies. `query-page`, `query-unavailable`, `record-issue` and `history-complete` accept only previously logged evidence. The automatically maintained `tracker-session-log.md` must remain beside the other run files.
- `begin` requires one explicit scope kind (`epic` or `tasks`), provider (`jira` or `sbertrek`), scope ids, label and source. Never infer the provider from key syntax. Execute only the exact TQL/JQL returned in `next_query`; `mcp-log` and `query-page` must reject any other search. Record every returned key and every full card, then use `collection-advance` to obtain the one exact cross-tracker query. Never replace this flow with a full-project inventory, title/description search or semantic candidate discovery. For each recorded issue, classify `assignee`, `estimate`, `epic` and `releases` as `value`, `absent` or `not-returned`; never turn an omitted field into confirmed absence. Use `run-status` and `tracker-session-log.md` to find gaps and never replace machine state with a hand-written snapshot.
...
- Do not proceed with an incomplete configuration, unfinished exact-query pagination, a missing card for any returned key, incomplete target-task history or unknown target-task participant. `set-participant` is valid only for the exact pending participant and `run_id`; ask all returned participant questions sequentially and accept only a roster `team_id`, never an invented role.
