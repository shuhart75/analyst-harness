...
- `сформируй пакет для разработки` is an end-to-end publication command. Before creating a revision, validate completeness, consistency, verifiability, impacts, derived slices, traceability and Russian language. Apply only meaning-preserving corrections automatically. For every semantic ambiguity ask the analyst exactly one question and wait; do not create or publish a package while any blocking question remains. On success create the transport and publish the revision directly as `sent` with `next_sdd_action.action = process`; there is no analyst-facing `ready` state.
