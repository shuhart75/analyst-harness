...
- Do not proceed with an incomplete configuration, a pending collection capability, an unresolved seed, an unread direct counterpart, a non-unique counterpart or an unknown participant. `set-participant` is valid only for the exact pending participant and `run_id`; ask all returned participant questions sequentially and accept only a roster `team_id`, never an invented role.
