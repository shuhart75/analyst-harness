...
- After `begin`, record metadata, each issue, each not-found direct read, each history event and each collection capability immediately through the matching `snapshot-*` command. Every seed must become a same-provider issue or an evidenced `snapshot-not-found` record. Every `complete` or `unavailable` collection claim requires evidence from an actual MCP call; never label a skipped call unavailable. Use `run-status` to find gaps. Do not replace this recording step with a manually assembled table or a large hand-written snapshot.
- Start tracker MCP calls only after a ready configuration and successful `begin` returning a `run_id`. Do not delegate tracker reading. Present task facts only after `reconcile` returns `tracker-read-reconciled`, `workflow_complete: true` and `final_response_allowed: true` for that same `run_id`. A failed guard forbids a manually assembled fallback summary.
