...
| `делаем требования` | `requirements` | Switch into living requirements mode; for an existing feature, first start its collaboration branch. |
| `начинаю работу над фичей <feature>` | `requirements` | Bootstrap collaboration and create `feature/<feature>/<analyst>` from current `origin/main` before reading or editing requirements. |
| `сохрани работу` | текущая feature-ветка | Проверить изменения, создать один смысловой коммит по точным путям и отправить только feature-ветку. |
| `обнови мою ветку` | текущая feature-ветка | Защищённо объединить актуальную `origin/main` с feature-веткой и отправить её. |
| `требования готовы к объединению` | текущая feature-ветка | Повторно проверить и отправить ветку; merge request создаёт и принимает человек. |
| `запрос на слияние принят` | операция с репозиторием | Доказать включение feature-коммита в `origin/main`, обновить локальную `main` и закрыть feature-сессию. |
...
| `сформируй пакет для разработки` | `requirements` | Сначала доказать актуальную чистую `main` без активной feature-сессии, затем проверить требования и отправить неизменяемую редакцию в SDD без ZIP. |
...
| `сходи в код` | текущий аналитический режим | Разрешить путь из реестра, зафиксировать состояние `code`, исследовать один контур, сообщить коммит и доказательства, затем проверить неизменность кодового репозитория. |
...
| `сходи в код` | Inspect current implementation facts in one registered `code` contour without changing it. | ad hoc answer or `features/*/.research/code-evidence.yaml`, then requirements when accepted |
