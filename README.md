...
- разработчик и LLM сверяют этот пакет с реальным кодом, тестами и архитектурой;
...
- после реализации факт возвращается в рабочий контур: задачи, проверки, actual-progress, релизный пакет и затем `baseline/current/`.
...
Это не план будущих работ. Если новая функциональность ещё не поставлена, она не попадает в `baseline/current/`. После финализации релиза подтверждённая дельта переносится в текущее состояние.

### 2. Планирование

`planning/` хранит intake, квартальные планы, командирские планы, приоритеты, состав команды, закрытые интервалы и план-факт. Функциональность планируется как законченный пользовательский или системный результат. На неё создаются ролевые planning stories: AN, BE, FE и QA, если соответствующая роль действительно участвует.

Утверждённые квартальный и командирский планы не меняются. Новая обнаруженная работа после утверждения появляется в actual-progress как черновик задачи или фактическая задача, чтобы в конце квартала можно было честно сравнить оценку с реальным выполнением.

### 3. Требования и дельта

`features/<feature>/requirements.md` является главным документом требований по функциональности. В нём описывается дельта относительно `baseline/current/`, включая:
...
- каждый пакет задачи фиксирует исходный путь, commit и хеши требований;
- LLM в режиме разработки читает пакет, открывает реальный код, находит затронутые модули, сверяет фактическую реализацию с требованиями и предлагает план реализации;
- если реальный код устроен иначе, чем предполагали требования, LLM не маскирует это в плане реализации, а возвращает вопрос или правку в требования;
...
Если требования и код лежат в одном монорепозитории, отдельный `dev-inbox` не нужен: пакет задачи может ссылаться на соседние файлы напрямую. Если кодовые репозитории отдельные, связь задаётся через `.workflow/code-repos.json`, а пакет задачи должен не копировать всё знание, а указывать источник и контрольные хеши.

### 6. Реализация, проверки и возврат факта

Цикл разработки идёт короткими итерациями:

1. LLM собирает пакет среза и проверяет свежесть требований.
2. LLM сравнивает требования с кодом и уточняет план реализации.
...
7. Завершённая работа отражается в actual-progress и релизном пакете.
8. После поставки релиза подтверждённая дельта переносится в `baseline/current/`.
